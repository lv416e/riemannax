...
def run_optimizer_comparison(problem, manifold, x0, problem_name: str, max_iterations: int = 100):
    ...
    optimizers: dict[str, dict[str, Any]] = {
        ...
        "RMomentum": {
            "method": "rmom",
            "options": {"learning_rate": 0.005, "momentum": 0.9, "max_iterations": max_iterations},
        },
    }
    ...
    for name, config in optimizers.items():
        ...
        try:
            result = rx.minimize(problem, x0, method=config["method"], options=config["options"])
            ...
            print(f"  FAILED: {e!s}")
            results[name] = {"success": False, "error": str(e), "optimization_time": time.time() - start_time}
    ...
    for i, (problem_name, problem, manifold, x0) in enumerate(problems_and_manifolds):
        ...
        for opt_name, (method, base_options) in optimizers.items():
            ...
            if method == "rsgd":
                ...
                init_fn, update_fn = rx.riemannian_adam(learning_rate=base_options["learning_rate"], beta1=base_options.get("beta1", 0.9), beta2=base_options.get("beta2", 0.999))
            elif method == "rmom":
                init_fn, update_fn = rx.riemannian_momentum(learning_rate=base_options["learning_rate"], momentum=base_options.get("momentum", 0.9))
...
def create_comprehensive_visualization(all_results: dict, problems_data: list):
    ...
    plt.figure(figsize=(20, 5 * n_problems))
    ...
    for i, (problem_name, results) in enumerate(all_results.items()):
        # Convergence plot
        plt.subplot(n_problems, 3, 3 * i + 1)

        for opt_name, opt_results in results.items():
            costs = opt_results["costs"]
            iterations = range(len(costs))
            plt.semilogy(iterations, costs, color=colors[opt_name], label=f"{opt_name}", linewidth=2, alpha=0.8)
        ...
        plt.subplot(n_problems, 3, 3 * i + 2)

        for opt_name, opt_results in results.items():
            costs = opt_results["costs"]
            times = opt_results["times"]
            plt.semilogy(times, costs, color=colors[opt_name], label=f"{opt_name}", linewidth=2, alpha=0.8)
        ...
        bars1 = plt.bar(
            x_pos - 0.2, final_costs, 0.4, label="Final Cost", color=[colors[name] for name in opt_names], alpha=0.7
        )

        # Normalize iterations for display
        max_iter = max(convergence_iterations)
        [it / max_iter for it in convergence_iterations]

        ax3_twin = ax3.twinx()
        bars2 = ax3_twin.bar(x_pos + 0.2, convergence_iterations, 0.4, label="Iterations", color="gray", alpha=0.5)
        ...
        for bar, cost in zip(bars1, final_costs, strict=False):
            ax3.text(
                bar.get_x() + bar.get_width() / 2, bar.get_height(), f"{cost:.2e}", ha="center", va="bottom", fontsize=8
            )

        for bar, iters in zip(bars2, convergence_iterations, strict=False):
            ax3_twin.text(
                bar.get_x() + bar.get_width() / 2, bar.get_height(), f"{iters}", ha="center", va="bottom", fontsize=8
            )
...
def demonstrate_optimizer_comparison():
    ...
    target_direction = jnp.array([0.0, 0.0, 1.0])  # North pole
